...
>
  {#each days as day}
    <div
      class="day text-gray-700 dark:text-gray-200"
      class:outside-month={!day.partOfMonth}
      class:is-today={day.isToday}
      class:is-disabled={!day.selectable}
    >
      <button
        class="day--label text-gray-700 dark:text-gray-200 bg-gray-100 dark:bg-gray-900"
...
<style>
  ...
  .dark .day.is-today .day--label,
  .dark .day.is-today .day--label:hover {
    opacity: 1;
    background: none;
    border-color: var(--highlight-color);
    color: #fff;
