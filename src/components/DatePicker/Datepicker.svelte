<script>
  ...
  export let formatForStore = "#{Y}#{m}#{d}";
  ...
  $: {
    ...
    fecForStore =
      typeof formatForStore === "function"
        ? formatForStore(selected)
        : formatDate(selected, formatForStore);
  }
  ...
  >
    <div slot="trigger">
      <slot {selected} {formattedSelected}>
        {#if !trigger}
          <button
            class="calendar-button text-gray-700 bg-gray-100 dark:bg-gray-900 dark:text-gray-200"
            type="button"
          >
            {formattedSelected}
          </button>
    ...
    <div slot="contents" class="rounded-lg">
      <div class="calendar">
        <NavBar
          {month}
          {year}
        ...
        />
        <div class="legend text-gray-700 dark:text-gray-400">
